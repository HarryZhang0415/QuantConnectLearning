...
In Initialize() create a manual StandardDeviation() indicator to calculate TSLA volatility for a 60 minute period. Save it to the variable self.volatility.
In OnData(), update our StandardDeviation() indicator manually with algorithm time using self.Time and TSLA's close price.
In OpeningBar, use IsReady to check if both volatility and the window are ready, or use return to wait for tomorrow.
Save the number of standard deviations of the delta to the variable deviations for use when we set holdings.
    # Gap as a number of standard deviations 
