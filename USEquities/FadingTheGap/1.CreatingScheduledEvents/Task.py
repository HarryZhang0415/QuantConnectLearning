...
Create an empty method ClosingBar. Create a scheduled event to run every day at TSLA market close that calls ClosingBar.
Create an empty method OpeningBar. Create a scheduled event to run every day at 1 minute after TSLA market open that calls OpeningBar.
Create a method ClosePositions and liquidate our position to limit our exposure and keep our holding period short. Create a scheduled event to run every day at 45 minutes after TSLA market open that calls ClosePositions.
"""
...
class FadingTheGap(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.Schedule.On(   self.DateRules.EveryDay(),
                            self.TimeRules.BeforeMarketClose("TSLA", 0),
                            self.ClosingBar)
        ...
        self.Schedule.On(   self.DateRules.EveryDay(),
                            self.TimeRules.AfterMarketOpen("TSLA", 45),
                            self.ClosePositions)
