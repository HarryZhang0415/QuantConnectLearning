'''
Task Objectives
Create a dictionary class variable named self.averages.
In your universe selection, check if the dictionary contains symbol.
If Symbol is not in the averages dictionary, create a new instance of SelectionData; and add it to averages.
Access the self.averages[symbol] and update it with the time and adjusted price data.
Using the SelectionData object in averages, check if the indicators are ready, and the fast-EMA is greater than the slow-EMA. Then use append to add symbol to the selected list.
'''
