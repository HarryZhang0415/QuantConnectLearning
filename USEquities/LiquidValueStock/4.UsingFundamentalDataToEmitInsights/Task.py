'''
Task Objectives
Let's define our Alpha Model class to emit insights once a month. We will emit a buy insight for the securities that have positive earning yields and emit a sell insight for the securities that have negative earning yields.

Create an instance of the LongShortEYAlphaModel in your Initialize() method
In the Alpha Model's Update() method, use an if statement to emit signals once a month
In the Alpha Model's Update() method, use a for loop to emit insights with insight directions based on whether earnings yield is greater or less than zero once a month
'''
